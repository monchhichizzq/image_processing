                    ...
                    for dname in dnames:
                        ...
                        for (dirpath_2, dirnames_2, filenames_2) in os.walk(p):
                            ...
                            for file in filenames_2_rearrange:
                                ...
                                # 列表list,元素全为0,'',false
                                if np.array(g_time_1D).any() == True:
                                    gesture_time_1D.append(g_time_1D)
                                t += 1
                            gesture_time_all_1D.append(gesture_time_1D)
                        ...
                        gesture_time_all_1D_padded = pad_sequences(gesture_time_all_1D, padding='post')
                        # print('gesture_all_1D_padded', gesture_all_1D_padded.shape)
                        # 这里我们得到了rnn_input （batch, time_length, joints）
                        # 我们要把他们分类 然后label
                        labels.append(label)
                label += 1
    n_channels = gesture_time_all_1D_padded.shape[2]
    n_classes = label
    seq_len = gesture_time_all_1D_padded.shape[1]
    print('seq_len', seq_len, 'n_classes', n_classes, 'n_channels', n_channels, 'class_names', class_names)
    return gesture_time_all_1D_padded, np.array(labels), n_channels, n_classes, seq_len, class_names
